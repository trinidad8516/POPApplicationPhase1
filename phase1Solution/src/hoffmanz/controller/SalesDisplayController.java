...
import hoffmanz.data.CashPayment;
import hoffmanz.data.CheckPayment;
import hoffmanz.data.CreditPayment;
import hoffmanz.data.Employee;
import hoffmanz.data.Payment;
import hoffmanz.data.Product;
import hoffmanz.model.SalesDisplayModel;
import hoffmanz.view.SalesDisplayView;
...
/**
 * Handles procedural logic for the sales display. This display will repeat the
 * order (selecting products to purchase) and payment phases for any number of
 * sales until the employee choses to log out.
 * 
 * @author hoffmanz
 * 
 */
	...
	private void removeProduct() {
		...
		if (match == -1) {
			...
			// over.
				...
				for (int i = match; i < invoiceSize - 1; i++) {
